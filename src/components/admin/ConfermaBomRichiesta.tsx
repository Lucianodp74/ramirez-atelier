...
type Bom = {
  ...
  richiestaId: string;
  stato: string;
  righe: Array<{ costoUnitario: number | null }>;
};
...
export function ConfermaBomRichiesta({ richiestaId }: { richiestaId: string }) {
  ...
  useEffect(() => {
    void carica().catch((e) => setError(e instanceof Error ? e.message : 'Impossibile leggere la BOM.'));
  }, [carica]);
  ...
  async function conferma() {
    ...
    setMessage(null);
    try {
      const response = await fetch('/api/admin/bom/conferma', {
        method: 'POST',
      ...
      setMessage('BOM confermata. Ora puoi salvare il preventivo.');
      await carica();
    } catch (e) {
      setError(e instanceof Error ? e.message : 'Errore conferma BOM');
    } finally {
      setBusy(false);
    }
  }
  ...
  return (
    <div className={`mt-4 rounded-md border p-4 ${confermata || chiusa ? 'border-slate-200 bg-slate-50' : 'border-emerald-200 bg-emerald-50'}`}>
