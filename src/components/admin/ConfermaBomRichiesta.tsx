...
export function ConfermaBomRichiesta({ richiestaId }: { richiestaId: string }) {
  ...
  async function carica() { const response = await fetch(`/api/admin/bom?richiestaId=${encodeURIComponent(richiestaId)}`); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? 'Impossibile leggere la BOM.'); const candidata = Array.isArray(data.data) ? data.data[0] : null; if (!candidata?.id) { setBom(null); return; } const dettaglioResponse = await fetch(`/api/admin/bom?id=${encodeURIComponent(candidata.id)}`); const dettaglio = await dettaglioResponse.json(); if (!dettaglioResponse.ok) throw new Error(dettaglio.error ?? 'Impossibile leggere il dettaglio della BOM.'); setBom({ id: dettaglio.id, richiestaId: dettaglio.richiestaId, stato: dettaglio.stato, righe: Array.isArray(dettaglio.righe) ? dettaglio.righe : [] }); }
  useEffect(() => { void carica().catch((e) => setError(e instanceof Error ? e.message : 'Impossibile leggere la BOM.')); }, [richiestaId]);
  async function conferma() { if (!bom) return; setBusy(true); setError(null); setMessage(null); try { const response = await fetch('/api/admin/bom/conferma', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ bomId: bom.id }) }); const data = await response.json(); if (!response.ok) throw new Error(data.error ?? 'Impossibile confermare la BOM.'); setMessage('BOM confermata. Ora puoi salvare il preventivo.'); await carica(); } catch (e) { setError(e instanceof Error ? e.message : 'Errore conferma BOM'); } finally { setBusy(false); } }
