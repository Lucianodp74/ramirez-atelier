...
import { DESCRIZIONI_IMMAGINI_PREVENTIVATORE, IMMAGINI_PREVENTIVATORE } from '@/components/preventivatore/immagini';
      ...
      <div className="mx-auto max-w-5xl px-4 py-5 sm:px-6 sm:py-8">
        ...
        <section className="rounded-2xl border border-border bg-card p-4 shadow-sm sm:p-8">
          {step === 1 && <div>
            ...
            <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{projectTypes.map((item) => { const selected = projectType === item.id; const immagine = (IMMAGINI_PREVENTIVATORE as Partial<Record<string, string>>)[item.id]; const descrizione = (DESCRIZIONI_IMMAGINI_PREVENTIVATORE as Partial<Record<string, string>>)[item.id] ?? item.text; return <button key={item.id} type="button" onClick={() => { setProjectType(item.id); if (item.id === 'ARMADIO') cambiaTipo('COLONNA'); if (item.id === 'LIBRERIA') cambiaTipo('LIBRERIA'); if (item.id === 'MADIA') cambiaTipo('BASE'); if (item.id === 'PARETE_ATTREZZATA') cambiaTipo('LIBRERIA'); if (item.id === 'MOBILE_BAGNO') cambiaTipo('BASE'); }} className={`${cardClass} ${selected ? 'border-foreground bg-muted/60' : 'border-border bg-background'}`}><div className="relative mb-7 h-20 overflow-hidden rounded-lg border border-border bg-muted">{immagine ? <Image src={immagine} alt={item.title} fill sizes="(max-width: 640px) 45vw, 260px" className="object-cover" /> : <div className="flex h-full items-end justify-between p-2 opacity-80"><span className="h-10 w-8 rounded-sm border-2 border-current" /><span className="h-16 w-8 rounded-sm border-2 border-current" /><span className="h-12 w-8 rounded-sm border-2 border-current" /></div>}</div><div className="flex items-center justify-between gap-2"><p className="text-base font-medium">{item.title}</p>{selected && <Check className="h-5 w-5" />}</div><p className="mt-1 text-xs leading-5 text-muted-foreground">{descrizione}</p></button>; })}</div>
          ...
          {step === 3 && <div>
            ...
            <p className="mb-3 text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">Materiale</p><div className="grid gap-3 sm:grid-cols-3">{catalogo.materiali.map((value) => { const selected = modulo.materiale === value; return <button key={value} type="button" onClick={() => aggiorna({ materiale: value })} className={`${cardClass} ${selected ? 'border-foreground bg-muted/60' : 'border-border bg-background'}`}><div className="relative mb-6 h-20 overflow-hidden rounded-lg border border-border bg-muted">{IMMAGINI_PREVENTIVATORE.VENATURE ? <Image src={IMMAGINI_PREVENTIVATORE.VENATURE} alt={materialLabels[value]} fill sizes="(max-width: 640px) 30vw, 180px" className="object-cover" /> : <div className="h-full w-full opacity-70 [background-image:repeating-linear-gradient(90deg,transparent,transparent_7px,currentColor_8px,currentColor_9px)]" />}</div><div className="flex items-center justify-between gap-2"><span className="font-medium">{materialLabels[value]}</span>{selected && <Check className="h-4 w-4" />}</div><span className="mt-1 block text-xs leading-5 text-muted-foreground">{materialDescriptions[value]}</span></button>; })}</div>
            <div className="mt-8 border-t border-border pt-7"><p className="mb-3 text-xs font-medium uppercase tracking-[0.18em] text-muted-foreground">Finitura</p><div className="grid gap-3 sm:grid-cols-3">{catalogo.finiture.map((value) => { const selected = modulo.finitura === value; return <button key={value} type="button" onClick={() => aggiorna({ finitura: value })} className={`${cardClass} ${selected ? 'border-foreground bg-muted/60' : 'border-border bg-background'}`}><div className="relative mb-6 h-20 overflow-hidden rounded-lg border border-border bg-gradient-to-br from-muted via-background to-muted">{IMMAGINI_PREVENTIVATORE.FINITURE && <Image src={IMMAGINI_PREVENTIVATORE.FINITURE} alt={finishLabels[value]} fill sizes="(max-width: 640px) 30vw, 180px" className="object-cover" />}</div><div className="flex items-center justify-between gap-2"><span className="font-medium">{finishLabels[value]}</span>{selected && <Check className="h-4 w-4" />}</div><span className="mt-1 block text-xs leading-5 text-muted-foreground">{finishDescriptions[value]}</span></button>; })}</div></div>
          </div>}
          {step === 4 && <div>
