...
export async function preparaSnapshotPreventivoBom(tenantId: string, bomId: string, pricing: BomPrezzoInput = {}): Promise<PreventivoBomSnapshot | null> {
  ...
  const righe = bom.righe.map((riga) => ({ id: riga.id, categoria: riga.categoria, codice: riga.codice, descrizione: riga.descrizione, unita: riga.unita, quantita: riga.quantita, costoUnitario: riga.costoUnitario, totaleCosto: riga.costoUnitario === null ? null : riga.quantita * riga.costoUnitario }));
  if (righe.some((riga) => riga.costoUnitario === null || !Number.isFinite(riga.costoUnitario))) throw new Error('Tutte le righe BOM devono avere un costo unitario prima del preventivo.');
...
export async function salvaPreventivoBom(tenantId: string, bomId: string, pricing: BomPrezzoInput = {}): Promise<PreventivoSalvato> {
  ...
  return db.$transaction(async (tx) => {
    await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtextextended(${`${tenantId}:${bomId}`}, 0))`;
    const richiesta = await tx.richiestaProgetto.findUnique({ where: { id: snapshot.richiestaId }, select: { id: true, tenantId: true, datiEstensione: true } });
    ...
    const storicoPrecedente = Array.isArray(estensione.preventiviStorico) ? estensione.preventiviStorico.filter((item): item is PreventivoStorico => Boolean(item && typeof item === 'object')) : [];
    ...
    const preventivo: PreventivoSalvato = { versione: ultimaVersione + 1, salvatoIl: new Date().toISOString(), bomId: snapshot.bomId, bomVersione: snapshot.versioneBom, pricing, prezzo: snapshot.prezzo };
    storico.push(preventivo);
    await tx.richiestaProgetto.update({ where: { id: richiesta.id }, data: { datiEstensione: { ...estensione, preventivo, preventiviStorico: storico } } });
    return preventivo;
  });
}
