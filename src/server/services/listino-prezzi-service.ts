...
export async function creaPrezzoListino(tenantId: string, dati: {
  ...
  return db.$queryRaw<VoceListino[]>`
    ...
    RETURNING "id", "categoria", "codice", "nome", "descrizione", "unita",
      "prezzo"::float8 AS "prezzo", "attivo", "createdAt", "updatedAt"
  `;
}
...
export async function aggiornaPrezzoListino(
  tenantId: string,
  id: string,
  dati: Partial<{ categoria: string; codice: string; nome: string; descrizione: string | null; unita: string; prezzo: number }>,
  motivo?: string,
) {
  const esistente = await db.$queryRaw<Array<{ prezzo: number }>>`
    SELECT "prezzo"::float8 AS "prezzo" FROM "listino_prezzo"
