...
export async function storicoPrezzoListino(
  tenantId: string,
  listinoPrezzoId: string,
): Promise<StoricoListino[]> {
  return db.$queryRaw<StoricoListino[]>`
    SELECT
      "id",
      "listinoPrezzoId",
      "prezzoPrecedente"::float8 AS "prezzoPrecedente",
      "prezzoNuovo"::float8 AS "prezzoNuovo",
      "motivo",
      "changedAt"
    FROM "listino_prezzo_storico"
    WHERE "tenantId" = ${tenantId}
      AND "listinoPrezzoId" = ${listinoPrezzoId}
    ORDER BY "changedAt" DESC
    LIMIT 20
  `;
}

export async function cercaPrezziListino(
  tenantId: string,
  query: string,
  unita?: string,
): Promise<VoceListino[]> {
...
export async function creaPrezzoListino(
  tenantId: string,
  dati: {
    categoria: string;
    codice: string;
...
export async function aggiornaPrezzoListino(
  tenantId: string,
  id: string,
  dati: Partial<{
    categoria: string;
  ...
  await db.$transaction(async (tx) => {
    await tx.$executeRaw`
      ...
      SET "categoria" = COALESCE(${dati.categoria ?? null}, "categoria"),
          ...
          "descrizione" = CASE
            WHEN ${dati.descrizione === undefined} THEN "descrizione"
            ELSE ${dati.descrizione ?? null}
          END,
    ...
    if (Math.abs(nuovoPrezzo - esistente[0].prezzo) > 0.000001) {
      await tx.$executeRaw`
        INSERT INTO "listino_prezzo_storico" (
          "id",
          "listinoPrezzoId",
          "tenantId",
          "prezzoPrecedente",
...
export async function impostaAttivoPrezzoListino(
  tenantId: string,
  id: string,
  attivo: boolean,
) {
