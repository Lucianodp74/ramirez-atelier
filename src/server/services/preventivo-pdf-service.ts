...
/** Assegna il numero di preventivo una sola volta per richiesta, in modo atomico per tenant/anno. */
export async function assegnaNumeroPreventivo(tenantId: string, richiestaId: string): Promise<{ numeroPreventivo: string; preventivoGeneratoIl: Date }> {
  return db.$transaction(async (tx) => {
    const esistente = await tx.richiestaProgetto.findUnique({ where: { id: richiestaId }, select: { tenantId: true, numeroPreventivo: true, preventivoGeneratoIl: true } });
    if (!esistente || esistente.tenantId !== tenantId) throw new Error('Richiesta non trovata.');
    if (esistente.numeroPreventivo && esistente.preventivoGeneratoIl) return { numeroPreventivo: esistente.numeroPreventivo, preventivoGeneratoIl: esistente.preventivoGeneratoIl };
    const anno = new Date().getFullYear();
    const prefisso = `${anno}-`;
    await tx.$executeRaw`SELECT pg_advisory_xact_lock(hashtextextended(${`${tenantId}:${anno}`}, 0))`;
    const dopoLock = await tx.richiestaProgetto.findUnique({ where: { id: richiestaId }, select: { numeroPreventivo: true, preventivoGeneratoIl: true } });
    if (dopoLock?.numeroPreventivo && dopoLock.preventivoGeneratoIl) return { numeroPreventivo: dopoLock.numeroPreventivo, preventivoGeneratoIl: dopoLock.preventivoGeneratoIl };
    const conteggio = await tx.richiestaProgetto.count({ where: { tenantId, numeroPreventivo: { startsWith: prefisso } } });
    const numeroPreventivo = `${prefisso}${String(conteggio + 1).padStart(3, '0')}`;
    const preventivoGeneratoIl = new Date();
    await tx.richiestaProgetto.update({ where: { id: richiestaId }, data: { numeroPreventivo, preventivoGeneratoIl } });
...
export interface DatiPreventivoPdf {
  ...
  preventivoCommerciale: { costoProduzione: number; costiAggiuntivi: number; ricaricoPercentuale: number; sconto: number; imponibile: number; iva: number; ivaPercentuale: number; totale: number } | null;
...
export async function datiPreventivoPdf(tenantId: string, richiestaId: string): Promise<DatiPreventivoPdf> {
  ...
  return {
    numeroPreventivo, dataEmissione: preventivoGeneratoIl,
    ...
    tipoProgettoNome: richiesta.tipoProgetto.nome, materiale: finitura?.nome ?? null, stile, ferramenta: ferramenta?.nome ?? null,
    larghezzaCm: form.larghezzaCm ?? null, profonditaCm: form.profonditaCm ?? null, messaggioLibero: richiesta.messaggioLibero,
    fasciaPrezzoMin: richiesta.fasciaPrezzoMin ? Number(richiesta.fasciaPrezzoMin) : null, fasciaPrezzoMax: richiesta.fasciaPrezzoMax ? Number(richiesta.fasciaPrezzoMax) : null,
    preventivoCommerciale: prezzo ? { costoProduzione: prezzo.costoProduzione, costiAggiuntivi: prezzo.costiAggiuntivi, ricaricoPercentuale: prezzo.ricaricoPercentuale, sconto: prezzo.sconto, imponibile: prezzo.imponibile, iva: prezzo.iva, ivaPercentuale: prezzo.ivaPercentuale, totale: prezzo.totale } : null,
...
async function recuperaPreventivoCommerciale(tenantId: string, richiestaId: string) {
  const righe = await db.$queryRaw<Array<{ id: string }>>`
    SELECT "id" FROM "bom" WHERE "tenantId" = ${tenantId} AND "richiestaId" = ${richiestaId} ORDER BY "versione" DESC LIMIT 1
  `;
