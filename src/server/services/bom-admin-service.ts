...
export async function aggiungiRigaBomAdmin(tenantId: string, bomId: string, input: CreaBomRigaInput) {
...
export async function eliminaRigaBomAdmin(tenantId: string, bomId: string, rigaId: string) {
  const result = await db.$executeRaw`
    ...
    WHERE br."id" = ${rigaId}
      ...
      AND b."stato" = 'BOZZA'
  `;
  if (result === 0) throw new Error('Riga non trovata o distinta non modificabile.');
}
