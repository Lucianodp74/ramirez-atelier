...
import { CODICI_TARIFFE_PREVENTIVATORE, DEFINIZIONI_TARIFFE_PREVENTIVATORE } from '@/lib/preventivatore/tariffe';
...
export { CODICI_TARIFFE_PREVENTIVATORE };
...
export function costruisciTariffeDaListino(voci: VoceTariffa[]): TariffePreventivatore {
  ...
  return {
    ...
    bordoEuroMl: numeroPositivo(trova(c.bordo), 'ML'), retroEuroM2: numeroPositivo(trova(c.retro), 'M2'), ferramentaPerPorta: numeroPositivo(trova(c.ferramentaPorta), 'PZ'), ferramentaPerCassetto: numeroPositivo(trova(c.ferramentaCassetto), 'PZ'), oreBase: numeroPositivo(trova(c.oreBase), 'H'), orePerM2: numeroPositivo(trova(c.orePerM2), 'H/M2'), orePerPorta: numeroPositivo(trova(c.orePerPorta), 'H/PZ'), orePerCassetto: numeroPositivo(trova(c.orePerCassetto), 'H/PZ'), orePerRipiano: numeroPositivo(trova(c.orePerRipiano), 'H/PZ'), costoOra: numeroPositivo(trova(c.costoOra), 'EUR/H'), ricaricoPercentuale: numeroPositivo(trova(c.ricarico), '%'),
  };
}
...
export async function caricaTariffePreventivatore(tenantId: string): Promise<TariffePreventivatore> {
  ...
  const voci = await db.$queryRaw<VoceTariffa[]>`SELECT "codice", "prezzo"::float8 AS "prezzo", "unita", "attivo" FROM "listino_prezzo" WHERE "tenantId" = ${tenantId} AND "attivo" = true AND "codice" IN (${Prisma.join(codici.map((codice) => Prisma.sql`${codice}`), ',')})`;
