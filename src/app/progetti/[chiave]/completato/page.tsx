...
import { recuperaRiepilogoConToken } from '@/server/services/richiesta-riepilogo-service';
...
export default async function CompletatoPage({
  ...
  const [{ chiave }, { id }] = await Promise.all([params, searchParams]);
  if (!id) notFound();

  const tokenRipresa = (await cookies()).get(`ra_bozza_${chiave}`)?.value;
  if (!tokenRipresa) notFound();

  let richiesta;
  try {
    richiesta = await recuperaRiepilogoConToken(id, tokenRipresa);
    ...
    <main className="flex min-h-screen flex-col items-center justify-center gap-6 bg-background px-6 text-center text-foreground">
      ...
      {richiesta.fasciaPrezzoMin !== null && richiesta.fasciaPrezzoMax !== null && (
        <div className="rounded-lg border border-border bg-card px-6 py-4">
          <p className="text-xs uppercase tracking-wide text-muted-foreground">Prima stima indicativa</p>
          ...
          <p className="mt-2 max-w-xs text-xs text-muted-foreground">
            Non è un preventivo definitivo: la cifra reale dipenderà dal progetto che definiremo insieme.
          </p>
        </div>
      )}
      <div className="max-w-sm rounded-lg border border-dashed border-border px-6 py-4">
        <p className="text-xs uppercase tracking-wide text-muted-foreground">Vuoi controllare a che punto siamo?</p>
        <p className="mt-2 text-sm text-muted-foreground">
          Salva questo link: potrai consultare lo stato del tuo progetto quando vuoi, senza bisogno di accedere a un account.
        </p>
        <a href={`/richiesta/${richiesta.tokenRipresa}`} className="mt-2 inline-block break-all text-sm text-accent underline">
