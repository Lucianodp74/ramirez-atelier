...
export async function POST(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body?.azione === 'elimina-riga') {
      if (typeof body.rigaId !== 'string') return NextResponse.json({ error: 'rigaId obbligatorio.' }, { status: 400 });
    ...
    if (body?.categoria == null || body?.descrizione == null || body?.quantita == null) {
      return NextResponse.json({ error: 'categoria, descrizione e quantita sono obbligatori.' }, { status: 400 });
    }
