...
export function PreventivoSummary({ costoProduzione }: Props) {
  ...
  const risultato = useMemo(
    () => calcolaPrezzoBom(costoProduzione, { ricaricoPercentuale: ricarico, costiFissi, lavorazioni, manodopera, spese, scontoPercentuale: sconto, ivaPercentuale: iva }),
  ...
  const campo = (label: string, value: number, setValue: (value: number) => void, step = '0.01') => (
    <label className="grid gap-1 text-sm">
      ...
      <input className="rounded border px-3 py-2" type="number" min="0" step={step} value={value} onChange={(event) => setValue(Number(event.target.value))} />
    </label>
  );
  ...
  return (
    <section className="mt-6 rounded-lg border bg-white p-5">
      <div className="mb-4">
        ...
        <p className="text-sm text-slate-500">Calcolo commerciale basato sul costo della BOM. I valori economici sono modificabili e non vengono salvati automaticamente.</p>
      </div>
