    ...
    <div className="mx-auto max-w-7xl px-6 py-12">
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          <Card>
            <CardHeader><CardTitle>Cliente e contatti</CardTitle></CardHeader>
            <CardContent className="grid gap-3 sm:grid-cols-2">
              <div><p className="text-xs text-muted-foreground">Nome</p><p className="text-sm">{richiesta.clienteNome ?? '—'}</p></div>
              <div><p className="text-xs text-muted-foreground">Email</p><p className="text-sm">{richiesta.clienteEmail ?? '—'}</p></div>
              <div><p className="text-xs text-muted-foreground">Telefono</p><p className="text-sm">{richiesta.clienteTelefono ?? '—'}</p></div>
              <div><p className="text-xs text-muted-foreground">Tipo cliente</p><p className="text-sm">{ETICHETTA_TIPO_CLIENTE[richiesta.clienteTipo] ?? richiesta.clienteTipo}</p></div>
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle>Il progetto</CardTitle></CardHeader>
            <CardContent>
              <RiepilogoRichiesta
              ...
              {richiesta.documenti && richiesta.documenti.length > 0 && (
                <div className="mt-4">
                  <p className="mb-2 text-sm font-medium text-muted-foreground">Dettaglio allegati</p>
                  <ul className="space-y-1">
                    {richiesta.documenti.map((d) => (
                      <li key={d.id} className="flex items-center justify-between text-sm">
                        <span>{d.nomeFileOriginale}</span>
                        <span className="text-xs text-muted-foreground">
                          <Badge variant="outline">{d.categoria}</Badge> · {formattaDimensione(d.dimensioneByte)}
                        </span>
                      </li>
                    ))}
                  </ul>
          ...
          <Card>
            ...
            <CardContent><NoteInterne richiestaId={richiesta.id} commenti={richiesta.commenti ?? []} /></CardContent>
          </Card>
            ...
            <CardHeader>
              ...
              {consultazioniCliente > 0 && (
                <p className="text-xs text-muted-foreground">
                  Il cliente ha consultato lo stato {consultazioniCliente} {consultazioniCliente === 1 ? 'volta' : 'volte'} tramite il link diretto (esperimento Portale Cliente).
                </p>
              )}
            </CardHeader>
            <CardContent><TimelineEventi eventi={richiesta.eventi ?? []} /></CardContent>
          </Card>
        </div>

        <div className="space-y-6">
          <Card>
            <CardHeader><CardTitle>Stato</CardTitle></CardHeader>
            <CardContent><ControlloCambioStato richiestaId={richiesta.id} statoCorrente={richiesta.stato} /></CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle>Completezza e prezzo</CardTitle></CardHeader>
            <CardContent className="space-y-3">
              <div><p className="text-xs text-muted-foreground">Livello di completezza</p><p className="text-lg font-semibold">{richiesta.indiceCompletezza}%</p></div>
              <div><p className="text-xs text-muted-foreground">Fascia di budget dichiarata</p><p className="text-sm">{richiesta.fasciaBudget?.nome ?? '—'}</p></div>
              <div>
              ...
              <div><p className="text-xs text-muted-foreground">Ricevuta il</p><p className="text-sm">{new Date(richiesta.createdAt).toLocaleString('it-IT')}</p></div>
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle>Priorità commerciale</CardTitle></CardHeader>
            <CardContent>
              {richiesta.prioritaCommerciale ? (
                <Badge variant={richiesta.prioritaCommerciale === 'ALTA' ? 'accent' : 'outline'}>{richiesta.prioritaCommerciale}</Badge>
              ) : (
                <p className="text-sm text-muted-foreground">Non ancora valutata (nessuna regola attiva ha prodotto un risultato per questa richiesta).</p>
              )}
            </CardContent>
          </Card>

          <Card>
            <CardHeader><CardTitle>Regole applicate</CardTitle></CardHeader>
            <CardContent>
                ...
                <ul className="space-y-3">
                  {esecuzioniRegole.map((e) => (
                    <li key={e.id} className="text-sm">
                      ...
                      <p className="text-xs text-muted-foreground">{ETICHETTA_ESITO_REGOLA[e.esito] ?? e.esito} · {new Date(e.createdAt).toLocaleString('it-IT')}</p>
                    </li>
                  ))}
                </ul>
              ) : (
                <p className="text-sm text-muted-foreground">Nessuna regola ancora valutata su questa richiesta.</p>
              )}
            </CardContent>
          </Card>
        </div>
