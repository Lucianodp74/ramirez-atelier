...
export default async function PreventivatoreCatalogoPage() {
  ...
  const configurato = requisiti.filter(([codice, , unita]) => { const voce = perCodice.get(codice); return Boolean(voce?.attivo && voce.unita === unita && voce.prezzo > 0); }).length;
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      ...
      <div className="grid gap-4 md:grid-cols-3"><Card><CardHeader><CardTitle>Moduli disponibili</CardTitle></CardHeader><CardContent><p className="text-3xl font-semibold">{CATALOGO_MODULI.length}</p><p className="text-xs text-muted-foreground">Configurazioni tecniche V2</p></CardContent></Card><Card><CardHeader><CardTitle>Tariffe configurate</CardTitle></CardHeader><CardContent><p className="text-3xl font-semibold">{configurato}/{requisiti.length}</p><p className="text-xs text-muted-foreground">Codici attivi con unità corretta e valore maggiore di zero</p></CardContent></Card><Card><CardHeader><CardTitle>Stato</CardTitle></CardHeader><CardContent><p className={`text-lg font-semibold ${configurato === requisiti.length ? 'text-foreground' : 'text-destructive'}`}>{configurato === requisiti.length ? 'Pronto al calcolo' : 'Listino da completare'}</p><p className="text-xs text-muted-foreground">Inserisci i valori Ramirez prima di usare il preventivo commerciale</p></CardContent></Card></div>
