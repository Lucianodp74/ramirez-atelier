...
export default async function PreventivatoreCatalogoPage() {
  ...
  const configurato = requisiti.filter(([codice, , unita]) => { const voce = perCodice.get(codice); return Boolean(voce?.attivo && voce.unita === unita); }).length;

  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      <Link href="/admin/catalogo" className="text-sm text-muted-foreground hover:underline">← Catalogo Tecnico</Link>
      <div className="mb-8 mt-2 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><h1 className="text-2xl font-semibold tracking-tight">Controllo Preventivatore</h1><p className="mt-2 max-w-3xl text-sm text-muted-foreground">Verifica tecnica del catalogo usato dal motore di stima. I prezzi vengono letti dal Listino reale; questa pagina non contiene prezzi di fallback.</p></div><Link href="/preventivatore" className="rounded-md border px-4 py-2 text-sm">Apri preventivatore cliente</Link></div>
      <div className="grid gap-4 md:grid-cols-3"><Card><CardHeader><CardTitle>Moduli disponibili</CardTitle></CardHeader><CardContent><p className="text-3xl font-semibold">{CATALOGO_MODULI.length}</p><p className="text-xs text-muted-foreground">Configurazioni tecniche V2</p></CardContent></Card><Card><CardHeader><CardTitle>Tariffe configurate</CardTitle></CardHeader><CardContent><p className="text-3xl font-semibold">{configurato}/{requisiti.length}</p><p className="text-xs text-muted-foreground">Codici attivi con unità corretta</p></CardContent></Card><Card><CardHeader><CardTitle>Stato</CardTitle></CardHeader><CardContent><p className={`text-lg font-semibold ${configurato === requisiti.length ? 'text-foreground' : 'text-destructive'}`}>{configurato === requisiti.length ? 'Pronto al calcolo' : 'Listino incompleto'}</p><p className="text-xs text-muted-foreground">Completa le voci mancanti dal Listino</p></CardContent></Card></div>
