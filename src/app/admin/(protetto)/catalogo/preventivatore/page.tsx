...
import { CODICI_TARIFFE_PREVENTIVATORE } from '@/server/services/preventivatore-listino-service';
...
const requisiti = [
  ['MAT-TRUCIOLARE', 'Truciolare', 'M2'],
  ['MAT-MDF', 'MDF', 'M2'],
  ['MAT-MULTISTRATO', 'Multistrato', 'M2'],
  ['FIN-MELAMINICO', 'Melaminico', 'M2'],
  ['FIN-LAMINATO', 'Laminato', 'M2'],
  ['FIN-LACCATO', 'Laccato', 'M2'],
  ['SERV-BORDO-ML', 'Bordatura', 'ML'],
  ['MAT-RETRO-M2', 'Retro', 'M2'],
  ['FER-PORTA', 'Ferramenta porta', 'PZ'],
  ['FER-CASSETTO', 'Ferramenta cassetto', 'PZ'],
  ['MAN-ORE-BASE', 'Ore base', 'H'],
  ['MAN-ORE-M2', 'Ore per m²', 'H/M2'],
  ['MAN-ORE-PORTA', 'Ore per porta', 'H/PZ'],
  ['MAN-ORE-CASSETTO', 'Ore per cassetto', 'H/PZ'],
  ['MAN-ORE-RIPIANO', 'Ore per ripiano', 'H/PZ'],
  ['MAN-COSTO-ORA', 'Costo orario', 'EUR/H'],
  ['COMM-RICARICO', 'Ricarico commerciale', '%'],
] as const;
...
export default async function PreventivatoreCatalogoPage() {
  ...
  const configurato = requisiti.filter(([codice]) => {
    const voce = perCodice.get(codice);
    return voce?.attivo && voce.unita === requisiti.find((r) => r[0] === codice)?.[2];
  }).length;
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      ...
      <section className="mt-8">
        <Card>
          <CardHeader><CardTitle>Tariffe richieste dal motore</CardTitle></CardHeader>
          <CardContent>
            <div className="overflow-x-auto">
              <table className="w-full min-w-[760px] text-sm">
                <thead className="border-b text-left text-muted-foreground"><tr><th className="p-3">Codice</th><th className="p-3">Voce</th><th className="p-3">Unità richiesta</th><th className="p-3">Stato</th><th className="p-3 text-right">Valore</th><th className="p-3">Azione</th></tr></thead>
                <tbody className="divide-y">
                  {requisiti.map(([codice, nome, unita]) => {
                    const voce = perCodice.get(codice);
                    const ok = Boolean(voce?.attivo && voce.unita === unita);
                    return <tr key={codice}><td className="p-3 font-mono text-xs">{codice}</td><td className="p-3">{nome}</td><td className="p-3">{unita}</td><td className="p-3">{ok ? '✓ Configurata' : '⚠ Da configurare'}</td><td className="p-3 text-right font-semibold">{ok && voce ? euro.format(voce.prezzo) : '—'}</td><td className="p-3"><Link href={`/admin/catalogo/listino?q=${encodeURIComponent(codice)}`} className="underline underline-offset-2">Apri Listino</Link></td></tr>;
                  })}
                </tbody>
              </table>
            </div>
      ...
      <section className="mt-8">
        <Card>
          <CardHeader><CardTitle>Famiglie di moduli</CardTitle></CardHeader>
          <CardContent><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{CATALOGO_MODULI.map((m) => <div key={m.codice} className="rounded-lg border p-4"><p className="font-medium">{m.nome}</p><p className="mt-1 text-xs text-muted-foreground">{m.min.larghezzaCm}–{m.max.larghezzaCm} × {m.min.altezzaCm}–{m.max.altezzaCm} × {m.min.profonditaCm}–{m.max.profonditaCm} cm</p><p className="mt-2 text-xs text-muted-foreground">{m.materiali.length} materiali · {m.finiture.length} finiture · {m.configurazioni.length} configurazioni</p></div>)}</div></CardContent>
        </Card>
      </section>
    </main>
  );
