...
export default async function PreventivatoreCatalogoPage() {
  ...
  const configurato = DEFINIZIONI_TARIFFE_PREVENTIVATORE.filter((def) => {
    const voce = perCodice.get(def.codice);
    return Boolean(voce?.attivo && voce.unita === def.unita && voce.prezzo > 0);
  }).length;
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      ...
      <div className="mb-8 mt-2 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        <div><h1 className="text-2xl font-semibold tracking-tight">Controllo Preventivatore</h1><p className="mt-2 max-w-3xl text-sm text-muted-foreground">Verifica tecnica del catalogo usato dal motore di stima. I prezzi vengono letti dal Listino reale; nessun prezzo demo viene usato come fallback.</p></div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        ...
        <Card><CardHeader><CardTitle>Tariffe configurate</CardTitle></CardHeader><CardContent><p className="text-3xl font-semibold">{configurato}/{DEFINIZIONI_TARIFFE_PREVENTIVATORE.length}</p><p className="text-xs text-muted-foreground">Codici attivi con unità corretta e valore maggiore di zero</p></CardContent></Card>
        <Card><CardHeader><CardTitle>Stato</CardTitle></CardHeader><CardContent><p className={`text-lg font-semibold ${configurato === DEFINIZIONI_TARIFFE_PREVENTIVATORE.length ? 'text-foreground' : 'text-destructive'}`}>{configurato === DEFINIZIONI_TARIFFE_PREVENTIVATORE.length ? 'Pronto al calcolo' : 'Listino da completare'}</p><p className="text-xs text-muted-foreground">Inserisci i valori Ramirez prima di usare il preventivo commerciale</p></CardContent></Card>
      </div>
      ...
      <section className="mt-8"><Card><CardHeader><CardTitle>Famiglie di moduli</CardTitle></CardHeader><CardContent><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5">{CATALOGO_MODULI.map((m) => <div key={m.codice} className="rounded-lg border p-4"><p className="font-medium">{m.nome}</p><p className="mt-1 text-xs text-muted-foreground">{m.min.larghezzaCm}–{m.max.larghezzaCm} × {m.min.altezzaCm}–{m.max.altezzaCm} × {m.max.profonditaCm} cm</p><p className="mt-2 text-xs text-muted-foreground">{m.materiali.length} materiali · {m.finiture.length} finiture · {m.configurazioni.length} configurazioni</p></div>)}</div></CardContent></Card></section>
    </main>
  );
}
