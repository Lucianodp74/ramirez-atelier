...
import {
  elencoFerramenta,
  categorieFerramentaEsistenti,
} from '@/server/services/ferramenta-service';
import {
  creaFerramentaAzione,
  aggiornaFerramentaAzione,
  impostaAttivaFerramentaAzione,
  eliminaFerramentaAzione,
...
export default async function FerramentaPage() {
  ...
  const [righe, categorie] = await Promise.all([
    ...
    categorieFerramentaEsistenti(contesto.tenantId),
  ]);
  const categorieForm = categorie.length > 0 ? categorie : ['cerniera'];
  ...
  return (
    <div className="mx-auto max-w-5xl px-6 py-12">
      ...
      <h1 className="mb-2 mt-2 text-2xl font-semibold tracking-tight">Ferramenta</h1>
      <p className="mb-8 text-sm text-muted-foreground">
        Cerniere, guide, sistemi di apertura e maniglie visibili nel configuratore.
      </p>
      ...
      <Card className="mb-8">
        <CardContent className="pt-6">
          <FormCatalogoSemplice
            ...
            azioneCrea={creaFerramentaAzione}
            azioneAggiorna={aggiornaFerramentaAzione}
          />
        </CardContent>
      </Card>
            ...
            <tbody>
              {righe.map((r) => (
                <RigaCatalogoSemplice
                  ...
                  azioneCrea={creaFerramentaAzione}
                  azioneAggiorna={aggiornaFerramentaAzione}
                  azioneImpostaAttiva={impostaAttivaFerramentaAzione}
                  azioneElimina={eliminaFerramentaAzione}
                />
              ))}
              {righe.length === 0 && (
                <tr>
                  <td colSpan={4} className="px-4 py-8 text-center text-muted-foreground">
                    Nessuna ferramenta ancora. Aggiungine una dal modulo sopra.
                  </td>
                </tr>
              )}
            </tbody>
