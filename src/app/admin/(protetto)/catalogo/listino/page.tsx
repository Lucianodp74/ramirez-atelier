...
export default async function ListinoPage() {
  ...
  const storici = await Promise.all(
    righe.map((riga) => storicoPrezzoListino(c.tenantId, riga.id)),
  );
    ...
    <main className="mx-auto max-w-6xl px-6 py-12">
      ...
      <div className="mb-8 mt-2">
        ...
        <p className="mt-2 max-w-3xl text-sm text-muted-foreground">
          I tuoi prezzi interni hanno priorità sui benchmark. Le modifiche al prezzo vengono
          storicizzate e non cambiano i costi già salvati nelle BOM.
        </p>
      </div>

      <form
        action={creaPrezzoListinoAzione}
        className="mb-8 grid gap-3 rounded-lg border p-4 sm:grid-cols-2 lg:grid-cols-6"
      >
        ...
        <label className="text-sm lg:col-span-5">
          ...
          <input
            name="descrizione"
            className="mt-1 w-full rounded-md border p-2"
            placeholder="Costo reale del fornitore"
          />
        </label>
        ...
        <table className="w-full min-w-[1100px] text-sm">
          <thead className="bg-secondary/40 text-left text-muted-foreground">
            <tr>
              ...
              <th className="p-3">Aggiorna prezzo</th>
            </tr>
          </thead>
            ...
            {righe.map((riga, indice) => {
              ...
              return (
                <tr key={riga.id}>
                  ...
                  <td className="p-3">
                    <form
                      action={aggiornaPrezzoListinoAzione}
                    ...
                    >
                      ...
                      <button
                        formAction={impostaAttivoPrezzoListinoAzione.bind(
                          null,
                          riga.id,
                          !riga.attivo,
                        )}
                      ...
                      >
                        {riga.attivo ? 'Disattiva' : 'Riattiva'}
                      </button>
                      {storico.length > 0 && (
                        <span className="text-xs text-muted-foreground">
                          Storico: {storico.length} modifiche
                        </span>
                      )}
                    </form>
                  </td>
            ...
            {righe.length === 0 && (
              <tr>
                <td colSpan={6} className="p-8 text-center text-muted-foreground">
                  Nessun prezzo personale inserito.
                </td>
              </tr>
            )}
          </tbody>
