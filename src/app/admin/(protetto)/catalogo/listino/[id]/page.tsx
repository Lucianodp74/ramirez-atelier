...
import { ComposizioneBomForm } from '@/app/admin/catalogo/listino/composizione-bom-form';
import {
...
export default async function ComposizioneListinoPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <main className="mx-auto max-w-7xl px-6 py-12">
      <Link href="/admin/catalogo/listino" className="text-sm text-muted-foreground hover:underline">← Listino Atelier</Link>
      ...
      <div className="mb-8 mt-2 flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="rounded-lg border bg-secondary/20 px-5 py-4 text-right"><div className="text-xs uppercase tracking-wide text-muted-foreground">Prezzo listino</div><div className="text-2xl font-semibold">{euro.format(dettaglio.composizione.prezzo)}</div></div>
      </div>
      ...
      <section className="mb-6 rounded-lg border p-5">
        <div className="mb-4"><h2 className="font-semibold">Aggiungi componente alla composizione</h2><p className="mt-1 text-sm text-muted-foreground">Il costo corrente del componente viene copiato nella distinta come snapshot. Le quantità restano modificabili.</p></div>
        <form action={aggiungiRigaComposizioneAzione} className="grid gap-3 md:grid-cols-[1fr_160px_auto]">
          ...
          <select name="componenteId" required className="rounded-md border bg-background p-2"><option value="">Seleziona materiale o componente…</option>{componenti.map((componente) => <option key={componente.id} value={componente.id}>{componente.nome} · {componente.unita} · {euro.format(componente.prezzo)}</option>)}</select>
      ...
      <section className="mb-6 rounded-lg border p-5">
        <div className="mb-4"><h2 className="font-semibold">Trasferisci la composizione in una BOM</h2><p className="mt-1 text-sm text-muted-foreground">Le righe vengono copiate nella BOM come fotografia della composizione attuale. La BOM deve essere in stato BOZZA.</p></div>
        {bomBozza.length === 0 ? <p className="text-sm text-muted-foreground">Nessuna BOM in bozza disponibile. Crea prima una BOM dalla relativa richiesta di progetto.</p> : dettaglio.righe.length === 0 ? <p className="text-sm text-muted-foreground">Aggiungi almeno un componente alla composizione prima di trasferirla.</p> : <ComposizioneBomForm composizioneId={id} bomBozza={bomBozza} />}
      </section>

      <section className="overflow-x-auto rounded-lg border">
        <table className="w-full min-w-[900px] text-sm"><thead className="bg-secondary/40 text-left text-muted-foreground"><tr><th className="p-3">Tipo</th><th className="p-3">Componente</th><th className="p-3">Unità</th><th className="p-3 text-right">Quantità</th><th className="p-3 text-right">Costo unitario snapshot</th><th className="p-3 text-right">Costo totale</th><th className="p-3">Azione</th></tr></thead>
          <tbody className="divide-y">{dettaglio.righe.map((riga) => <tr key={riga.id}><td className="p-3">{riga.tipo === 'MATERIALE' ? 'Materiale' : 'Componente'}</td><td className="p-3"><div className="font-medium">{riga.nome}</div><div className="text-xs text-muted-foreground">{riga.codice}</div></td><td className="p-3">{riga.unita}</td><td className="p-3 text-right"><form action={aggiornaRigaComposizioneAzione} className="flex justify-end gap-2"><input type="hidden" name="id" value={riga.id} /><input type="hidden" name="composizioneId" value={id} /><input name="quantita" type="number" min="0.001" step="0.001" defaultValue={riga.quantita} className="w-24 rounded-md border p-2 text-right" /><button className="rounded-md border px-3 py-2">Salva</button></form></td><td className="p-3 text-right">{euro.format(riga.costoUnitario)}</td><td className="p-3 text-right font-semibold">{euro.format(riga.costoTotale)}</td><td className="p-3"><form action={rimuoviRigaComposizioneAzione}><input type="hidden" name="id" value={riga.id} /><input type="hidden" name="composizioneId" value={id} /><button className="rounded-md border px-3 py-2 text-sm">Rimuovi</button></form></td></tr>)}{dettaglio.righe.length === 0 && <tr><td colSpan={7} className="p-10 text-center text-muted-foreground">Nessun componente inserito. Aggiungi materiali e ferramenta per costruire la distinta della composizione.</td></tr>}</tbody>
        </table>
      </section>
