...
const progettiHome = [
  { immagine: '/progetti/cucina-su-misura.jpg', titolo: 'Cucine su misura', descrizione: 'Proporzioni, materiali e funzioni pensati intorno al tuo modo di vivere.', href: '/progetti/cucine' },
  { immagine: '/progetti/armadio-cabina.jpg', titolo: 'Cabine e armadi', descrizione: 'Contenimento sartoriale, dal primo disegno all’ultimo dettaglio.', href: '/progetti/armadi-e-cabine-armadio' },
  { immagine: '/progetti/zona-giorno-su-misura.jpg', titolo: 'Zona giorno', descrizione: 'Pareti attrezzate, librerie e arredi che dialogano con lo spazio.', href: '/progetti/zona-giorno' },
];
      ...
      <section className="bg-background px-6 py-20 sm:py-28">
        <div className="mx-auto max-w-7xl">
          <div className="mb-10 flex items-end justify-between gap-8 sm:mb-14">
            <div><p className="mb-4 text-xs uppercase tracking-[0.25em] text-[#A6532B]">I progetti</p><h2 className="font-serif text-4xl font-light tracking-tight sm:text-5xl">Spazi pensati intorno a te.</h2></div>
            <Link href="/progetti" className="hidden pb-1 text-xs uppercase tracking-[0.22em] text-[#8F4525] transition-opacity hover:opacity-60 sm:block">Vedi tutti i progetti →</Link>
          </div>
          <div className="grid gap-5 lg:grid-cols-3">
            {progettiHome.map((progetto) => (
              <Link key={progetto.titolo} href={progetto.href} className="group overflow-hidden rounded-[1.25rem] border border-border bg-card shadow-[0_18px_55px_rgba(42,38,34,0.07)] transition-shadow duration-500 hover:shadow-[0_24px_70px_rgba(42,38,34,0.11)]">
                <article>
                  <div className="relative aspect-[3/2] overflow-hidden bg-muted"><Image src={progetto.immagine} alt={progetto.titolo} fill sizes="(min-width:1024px) 33vw,(min-width:640px) 50vw,100vw" quality={90} className="object-cover transition-transform duration-700 group-hover:scale-[1.025]" /></div>
                  <div className="p-6 sm:p-7"><h3 className="font-serif text-2xl font-light tracking-tight sm:text-[1.75rem]">{progetto.titolo}</h3><p className="mt-3 max-w-sm text-sm leading-relaxed text-muted-foreground">{progetto.descrizione}</p><span className="mt-6 inline-flex text-xs uppercase tracking-[0.2em] text-[#A6532B]">Scopri →</span></div>
                </article>
              </Link>
            ))}
          </div>
          <Link href="/progetti" className="mt-8 inline-block text-xs uppercase tracking-[0.22em] text-[#8F4525] sm:hidden">Vedi tutti i progetti →</Link>
        </div>
      </section>
