  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      <nav className="mx-auto flex min-h-[160px] max-w-6xl items-center justify-center px-6 py-8 sm:min-h-[190px] sm:py-10">
        <Link href="/" aria-label="Ramirez Atelier" className="block"><Image src="/logo-completo.png" alt="Ramirez Atelier — Arredi su misura" width={320} height={178} priority className="h-auto w-[250px] sm:w-[320px]" /></Link>
      </nav>
      <section className="relative overflow-hidden border-y border-border/70 bg-[radial-gradient(circle_at_78%_48%,rgba(166,83,43,0.10),transparent_34%),linear-gradient(135deg,rgba(166,83,43,0.05),transparent_42%)]"><div className="mx-auto grid max-w-7xl items-center gap-4 px-6 lg:grid-cols-[0.78fr_1.22fr] lg:px-10"><div className="relative z-10 py-16 sm:py-20 lg:py-24"><p className="mb-6 text-xs uppercase tracking-[0.25em] text-[#A6532B]">Falegnameria artigiana — dal 1987</p><p className="mb-5 font-serif text-sm italic text-[#A6532B]">STELLA · Madia manifesto</p><h1 className="text-balance font-serif text-[2.8rem] font-light leading-[1.04] tracking-tight sm:text-6xl xl:text-7xl">La tua casa,<br />disegnata a mano,<br /><span className="italic text-[#A6532B]">costruita per durare.</span></h1><p className="mt-7 max-w-xl text-balance text-lg leading-relaxed text-muted-foreground">Un oggetto di falegnameria contemporanea, esplorabile in 3D. Materiali, proporzioni e dettagli nascono dal dialogo tra progetto e materia.</p><div className="mt-9 flex flex-wrap items-center gap-4"><Button size="lg" variant="accent" className="border-[#A6532B] bg-[#A6532B] text-white shadow-[0_10px_30px_rgba(166,83,43,0.22)] hover:bg-[#8F4525]" asChild><Link href="/progetti">Inizia il tuo progetto</Link></Button><span className="text-xs uppercase tracking-[0.18em] text-[#8F4525]">Modello interattivo</span></div></div><div className="-mx-6 sm:-mx-8 lg:mx-0"><StellaHero3D /></div></div></section>
      <section className="border-b border-border bg-card py-20 sm:py-24"><div className="mx-auto max-w-6xl px-6"><div className="grid items-center gap-10 lg:grid-cols-[1.05fr_0.95fr] lg:gap-16"><div className="overflow-hidden rounded-[1.25rem] border border-border/70 bg-background shadow-[0_20px_60px_rgba(42,38,34,0.08)]"><Image src="/foto-finiture.jpg" alt="Ventaglio di finiture e materiali disponibili - legni, laccati, pietre" width={1536} height={1024} quality={90} className="aspect-[4/3] h-full w-full object-cover transition-transform duration-700 hover:scale-[1.02]" /></div><div className="max-w-xl"><p className="mb-4 text-xs uppercase tracking-[0.25em] text-[#A6532B]">La materia</p><h2 className="font-serif text-4xl font-light tracking-tight sm:text-5xl">Ogni progetto comincia da qui.</h2><p className="mt-6 text-lg leading-relaxed text-muted-foreground">Rovere o noce, laccato opaco o pietra naturale — decine di finiture e colorazioni personalizzabili, per un progetto che sia davvero il tuo. Le vedrai tutte, una a una, nel momento in cui inizierai a costruire la tua idea con noi.</p><div className="mt-8 h-px w-16 bg-[#A6532B]" aria-hidden="true" /></div></div></div></section>
