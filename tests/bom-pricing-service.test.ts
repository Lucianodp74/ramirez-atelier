...
describe('calcolaPrezzoBom', () => {
  ...
  it('calcola separatamente lavorazioni, manodopera e spese prima del ricarico', () => {
    expect(calcolaPrezzoBom(1000,{lavorazioni:150,manodopera:200,spese:50,ricaricoPercentuale:25})).toMatchObject({costoProduzione:1000,costiAggiuntivi:400,baseConRicarico:1650,totale:1650});
  });
  it('usa zero come default per i parametri economici omessi', () => expect(calcolaPrezzoBom(125)).toMatchObject({costoProduzione:125,costiFissi:0,costiAggiuntivi:0,totale:125,ricaricoPercentuale:0,ivaPercentuale:0}));
  it('rifiuta percentuali e importi non validi', () => { expect(()=>calcolaPrezzoBom(100,{ricaricoPercentuale:101})).toThrow(); expect(()=>calcolaPrezzoBom(100,{scontoPercentuale:-1})).toThrow(); expect(()=>calcolaPrezzoBom(100,{ivaPercentuale:Number.NaN})).toThrow(); expect(()=>calcolaPrezzoBom(-1)).toThrow(); expect(()=>calcolaPrezzoBom(100,{costiFissi:-1})).toThrow(); expect(()=>calcolaPrezzoBom(100,{lavorazioni:-1})).toThrow(); expect(()=>calcolaPrezzoBom(100,{manodopera:-1})).toThrow(); expect(()=>calcolaPrezzoBom(100,{spese:-1})).toThrow(); });
});
